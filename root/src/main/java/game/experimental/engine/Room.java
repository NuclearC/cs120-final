...
public class Room implements Settings {
    ...
    // monotonic timer that gets increased every frame
            ...
            if (playerEntities[i] != null) {
                ...
                for(int j =0 ; j < player.getProjectiles().size(); j ++) {
                    if(player.getProjectiles().get(j) != null)
                        ...
                        System.out.println(player.getProjectiles().get(j));
                }
                ...
                for(int j =0 ; j < player.getProjectiles().size(); j ++) {
                    ...
                    if (projectile != null && projectile.remainsWithinBoundary(projectile.getPosition()))
                        quadTree.insert(player.getProjectiles().get(j), new BoundingBox(player.getProjectiles().get(j).getPosition(), player.getProjectiles().get(j).size));
                    else{
    ...
    /**
     * Creates a new Player with the given id and with the id of the Client with
     * whom the player is associated.
     * 
    ...
    private PlayerEntity createPlayer(int id, int ownerID) {
        ...
        PlayerEntity player = new PlayerEntity(tickCount, playerPosition, id, ownerID);// add function to get the position vector
    ...
    /**
     * creates MovingCollectable with the give id and type in random position(we
     * need to change this!!!!!!)
     * 
    ...
    /**
     * creates Static Collectable with the give id and type in random position(we
     * need to change this!!!!!!)
     * 
     * @param id   the id of the collectable
     * @param type the type of the collectable
    ...
    private CollectableEntity createStaticCollectable(int id, int type) {
        Vector2F position = Vector2F.randomVector(0, MAP_HEIGHT, 0, MAP_HEIGHT); // TODO
        return new CollectableEntity(tickCount, position, id, -1, 1);// TODO
    }
